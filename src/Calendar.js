...
class Calendar extends Component {
	  ...
	  deleteItem = (index) => {
	    let Mon = this.state.Monday; //get the array from state
	    var i = Mon.indexOf(index);
	    Mon.splice(i, 1); //cutout the value at that index
	    this.setState({Mon}); //set the state
	  	const db = firebase.firestore();	    
	    const docRef = db.collection('todos').doc('Monday').collection('tasks');
	...
	render() { 
	    let Mon = this.state.Monday.map((value, index) =>
	...
	return (
					  ...
					  <Collapsible trigger={"Monday" || <Collapsible />} classParentString="monCal cal" className="cal wow fadeInLeft animated" data-wow-delay="1s">
