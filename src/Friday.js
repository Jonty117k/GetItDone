...
class Friday extends Component {
	  ...
	  deleteItem = (index) => {
	    let Mon = this.state.Friday; //get the array from state
	    var i = Mon.indexOf(index);
	    Mon.splice(i, 1); //cutout the value at that index
	    this.setState({Mon}); //set the state
	  	const db = firebase.firestore();	    
	    const docRef = db.collection('todos').doc('Friday').collection('tasks');
	...
	render() { 
	    let Fri = this.state.Friday.map((value, index) =>
	...
	return (
					  ...
					  <Collapsible trigger="Friday" classParentString="monCal cal" className="wow fadeInLeft animated" data-wow-delay="1s">
