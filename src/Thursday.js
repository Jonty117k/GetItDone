...
class Thursday extends Component {
	  ...
	  deleteItem = (index) => {
	    let Mon = this.state.Thursday; //get the array from state
	    var i = Mon.indexOf(index);
	    Mon.splice(i, 1); //cutout the value at that index
	    this.setState({Mon}); //set the state
	  	const db = firebase.firestore();	    
	    const docRef = db.collection('todos').doc('Thursday').collection('tasks');
	...
	render() { 
	    let Thurs = this.state.Thursday.map((value, index) =>
	...
	return (
					  ...
					  <Collapsible trigger="Thursday" classParentString="monCal cal" className="wow fadeInLeft animated" data-wow-delay="1s">
